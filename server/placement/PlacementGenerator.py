...
def generateValidPlacements(board, piece):
    ...
    for x in range(board.width):
        for o in range(4):
            y = getDropY(board, piece, x, o)
            if y is not None and pieceFits(board, piece, x, y, o):
...
def getDropY(board, piece, x, orientation):
    for y in range(board.height):
        if not pieceFits(board, piece, x, y, orientation):
            if y == 0:
                return None  # no space to drop
            return y - 1  # last valid spot
    return board.height - 1  # can go all the way to the bottom
